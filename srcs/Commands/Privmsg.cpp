...
void HDE::SocketHde::Privmsg(std::vector<std::string> message, int i)
{
    ...
    else
    {
        if(GetUserIdByName(AllUsers, message[1]) != -1)
        {
            ...
            std::string username = clt.at(fds[k].fd).getUsername();
            std::string selfStr = ":" + nick + "!" + username + "@" + clt.at(fds[i].fd).getLocalhost() + " PRIVMSG " + clt.at(fds[i].fd).getNickname() + " :"+ message[2] + "\r\n";
